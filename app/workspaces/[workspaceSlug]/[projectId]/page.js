...
const TaskCard         = dynamic(() => import("@/components/taskCard"),         { ssr: false });
    ...
    useEffect(() => {
        ...
        const unsubscribe = onSnapshot(q, (querySnapshot) => {
            if (!querySnapshot.empty) {
                ...
                setWorkspace(wsData);
                setProject(wsData.projects?.find(p => p.id === projectId) ?? null);
            } else {
                setWorkspace(null);
                setProject(null);
            }
            ...
            <div className="container mx-auto px-6 py-10 max-w-7xl min-h-screen">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-[1fr_260px] gap-5 items-start">
                    <div className="flex flex-col gap-5">
                        <div className="h-48 w-full bg-card border border-border/50 rounded-xl animate-pulse" />
                        ...
                        <div className="flex flex-col gap-2.5 text-sm">
                            <div className="flex justify-between items-center">
                                <span className="text-muted-foreground flex items-center gap-2">
                                    <Circle className={statusStyles[project?.status]} size={16} />
                                    Status
                                </span>
                            ...
                            <div className="flex justify-between items-center">
                                ...
                                <span className="text-foreground">{tasks.length} total</span>
                            </div>
                            {tasks.length > 0 && (
                                <>
                                    <div className="h-px bg-border/50" />
                                    <div className="flex justify-between items-center">
                                        <span className="text-muted-foreground flex items-center gap-2"><CheckSquare className="w-4 h-4 text-emerald-500" />Done</span>
                                        <span className="text-foreground">{doneTasks} / {tasks.length}</span>
                                    </div>
                                </>
                            )}
                        </div>
