...
import { Layout, Grid, Users, Calendar, ArrowLeft, Plus, Sparkles } from "lucide-react"
...
export default function WorkspacePage() {
    ...
    }, [user, authLoading, param.workspaceSlug])
    ...
    return (
        <>
                            ...
                            <div className="flex flex-wrap justify-center lg:justify-start gap-6">
                                {[
                                    ...
                                    <div key={i} className="flex items-center gap-4 bg-card/40 backdrop-blur-md border border-border/40 p-5 rounded-3xl shadow-sm min-w-40 hover:border-vertex-primary/30 transition-colors group/stat">
                                        <div className={`p-2.5 bg-${stat.color}/10 rounded-2xl text-${stat.color} group-hover/stat:scale-110 transition-transform`}>
                ...
                <section className="mt-12">
                    ...
                    {(!workspace.projects || workspace.projects.length === 0) ? (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                            {workspace.projects.map((projectId, idx) => (
                                <motion.div 
                                    key={projectId}
                                ...
                                >
                                    <div className="bg-card border border-border/50 rounded-[2.5rem] p-8 h-full hover:shadow-2xl hover:shadow-vertex-primary/5 hover:border-vertex-primary/20 transition-all duration-500 cursor-pointer">
                                        <div className="flex justify-between items-start mb-6">
                                            <div className="p-4 bg-muted rounded-2xl text-muted-foreground group-hover/proj:bg-vertex-primary group-hover/proj:text-primary-foreground transition-all duration-500 group-hover/proj:scale-110 group-hover/proj:rotate-3">
                                                <Grid className="w-7 h-7" />
                                            </div>
                                            <div className="bg-emerald-500/10 text-emerald-500 text-[10px] font-black uppercase px-3 py-1.5 rounded-full border border-emerald-500/20">
                                                Active
                                            </div>
                                        </div>
                                        <h3 className="text-2xl font-black mb-2 group-hover/proj:text-vertex-primary transition-colors">Project {idx + 1}</h3>
                                        <p className="text-muted-foreground text-sm line-clamp-2 leading-relaxed">
                                            This is a project placeholder. You can manage tasks, codebase, and deployment for this project.
                                        </p>
                                    </div>
                                </motion.div>
                            ))}
            ...
            <CreateProjectDialog open={createProjectDialogOpen} setOpen={setCreateProjectDialogOpen} workspaceId={workspace.id}/>
        </>
    )
}